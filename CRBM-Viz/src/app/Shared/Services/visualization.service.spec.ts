...
describe('VisualizationService', () => {
  ...
  it('should return an Observable<Visualization>', () => {
    ...
    service.getVisualization('1').subscribe(vizs => {
      expect(vizs[0].length).toEqual(1);
      expect(vizs[0].id).toEqual(dummyViz.id);
      expect(vizs[0].name).toEqual(dummyViz.name);
      expect(vizs[0].schema.spec).toEqual(dummyViz.spec);
    });

...
export class SimulationIdMapService {
    constructor(@InjectModel(SimulationIdMap.name) private simIdMapModel: Model<SimulationIdMap>)
        {}

    async create(createSimIdMap: {uuid: string, projectName: string}) : Promise<{uuid: string, projectName: string} {

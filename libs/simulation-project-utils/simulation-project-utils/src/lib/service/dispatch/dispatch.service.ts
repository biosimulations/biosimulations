...
export type OntologyTermIdNameUrlMap = {
  [ontologyId: string]: { [termId: string]: OntologyTermIdNameUrl };
};
...
export class DispatchService {
  ...
  public getSimulatorsFromDb(): Observable<SimulatorsData> {
    ...
    const endpoint = this.endpoints.getSimulatorsEndpoint(true, undefined, undefined, false);
            ...
            for (const simulator of simulators) {
              if (simulator?.image && simulator?.biosimulators?.validated) {
                ...
                simulator.algorithms.forEach((algorithm: any): void => {
                  simulatorSpecsMap[simulator.id].modelingFrameworksAlgorithmsForModelFormats.push({
                    formatEdamIds: algorithm.modelFormats.map((format: any): void => {
                      return format.id;
                    }),
                    frameworkSboIds: algorithm.modelingFrameworks.map((framework: any): void => {
                      return framework.id;
                    }),
                    ...
                    parameters: algorithm.parameters.map((param: any): AlgorithmParameter => {
                      return {
                        ...
                        formattedValue: param.value
                          ? formatValue(param.type, parseValue<string>(getKisaoTermName, param.type, param.value)) || ''
                          : '',
                        ...
                        formattedRecommendedRange: param.recommendedRange
                          ? param.recommendedRange.map((value: string): string => {
                              return formatValue(
                                param.type,
                                parseValue<string>(getKisaoTermName, param.type, value),
                              ) as string;
                            })
                          : ['--none--'],

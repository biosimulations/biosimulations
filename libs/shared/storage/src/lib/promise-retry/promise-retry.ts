...
export function isRetryError(error: any) {
  return (
    error && error.code === 'EPROMISERETRY' && hasOwn.call(error, 'retried')
  );
}
...
export function promiseRetry<T>(
  fn: (retry: RetryFn, number: number) => Promise<T>,
      ...
      Promise.resolve()
        .then(function () {
          return fn(function (error: any) {
            ...
            throw errcode.default(new Error('Retrying'), 'EPROMISERETRY', {
              retried: error,
            });
          }, number);
        })

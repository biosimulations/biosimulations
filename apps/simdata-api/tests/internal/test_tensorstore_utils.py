...
async def test_extract_hdf5_metadata(driver: TS_DRIVER, kvstore_driver: KV_DRIVER):
    ...
    await write_ts_metadata_root(driver=driver, kvstore_driver=kvstore_driver, kvstore_root_path=store_path, hdf5_file=hdf5_file)
    for group in hdf5_file.groups:
        for dataset in group.datasets:
            ...
            await write_ts_dataset(driver=driver, kvstore_driver=kvstore_driver, kvstore_path=store_path / Path(dataset.name),
    ...
    for group in hdf5_file.groups:
        for dataset in group.datasets:
            ...
            array, attr = await read_ts_dataset(driver=driver, kvstore_driver=kvstore_driver, kvstore_path=store_path / Path(dataset.name))

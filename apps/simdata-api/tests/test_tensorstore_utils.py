...
from simdata_api.internal.tensorstore_utils import write_dataset, write_metadata_root, TS_DRIVER, KV_DRIVER, \
    read_from_local_ts
...
async def test_extract_hdf5_metadata(driver: TS_DRIVER, kvstore_driver: KV_DRIVER):
    ...
    await write_metadata_root(driver=driver, kvstore_driver=kvstore_driver, kvstore_root_path=store_path, hdf5_file=hdf5_file)
    ...
    for group in hdf5_file.groups:
        for dataset in group.datasets:
            ...
            array, attr = await read_from_local_ts(driver=driver, kvstore_driver=kvstore_driver, kvstore_path=store_path/Path(dataset.name))
...
async def test_simple_dataset(driver: TS_DRIVER, kvstore_driver: KV_DRIVER):
    ...
    await write_dataset(driver=driver, kvstore_driver=kvstore_driver, kvstore_path=test_file_path,
    ...
    # read the data and verify that it matches the test data:
    array, attr = await read_from_local_ts(driver=driver, kvstore_driver=kvstore_driver, kvstore_path=test_file_path)
